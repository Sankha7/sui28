<?php
//full response
require '../merchant/connection.php';
...
if ($payment_status == 000) {
	$sql = mysqli_query($con, "UPDATE orders SET txn_id= '$transaction_ref', payment_status = 'success', modified = '$transaction_datetime' WHERE order_id = '$order_id'");
	$_SESSION['payment_status'] = 1;
	unset($_SESSION['cart']);
	header("location:../index");
} else if ($payment_status == 001) {
	$sql = mysqli_query($con, "UPDATE orders SET txn_id= '$transaction_ref', modified = '$transaction_datetime' WHERE order_id = '$order_id'");
	$_SESSION['payment_status'] = 2;
	unset($_SESSION['cart']);
	header("location:../index");
} else {
	$sql = mysqli_query($con, "UPDATE orders SET txn_id= '$transaction_ref', payment_status = 'failed', modified = '$transaction_datetime' WHERE order_id = '$order_id'");
